...
class initialize_network_FCQN:
    def __init__(self, plot_directory: str, save_weights_dir: str, custom_load_path=None):
        ...
        with self.g.as_default():
            ...
            self.lstmstate = tf.placeholder(tf.float32, shape=[1, 2, 52**2])
            ...
            self.model = FCQN(self.X, self.lstmstate)
    ...
    def Q_val(self, curr_state_tuple):
        grids, curr_positions, prev_positions, lstm_state = curr_state_tuple
        return self.sess.run([self.predict,self.lstm_state_output],
    ...
    def train_n(self, curr_state_tuple, action, target_Q, batch_size, lr, epsilon, iter):
        ...
        _, loss, Q = self.sess.run([self.train, self.loss, self.predict],
                                   feed_dict={self.batch_size: batch_size, self.learning_rate: lr, self.sampled_grids: grids,
                                              self.current_position_map: curr_positions, self.previous_position_map: prev_positions,
                                              self.action: action, self.target: target_Q, self.lstmstate: lstm_state})
        meanQ = np.mean(Q)
        maxQ = np.max(Q)
        # Log to tensorboard
    ...
    def action_selection(self, curr_state_tuple):
        ...
        qvals = self.sess.run(self.predict,
                              feed_dict={self.batch_size: grids.shape[0],
                                         ...
                                         self.previous_position_map: prev_positions, self.lstmstate: lstm_state})
    ...
    def action_selection_non_repeat(self, curr_state_tuple, previous_actions):
        ...
        grids, curr_positions, prev_positions, lstm_state = curr_state_tuple
        qvals, lstm_state_out = self.sess.run([self.predict,self.lstm_state_output],
                              feed_dict={self.batch_size: grids.shape[0],
                                         self.sampled_grids: grids, self.current_position_map: curr_positions,
                                         self.previous_position_map: prev_positions, self.lstmstate: lstm_state})
        while True:
            ...
            if repeated_action:
                ...
                return np.array([action_idx]), lstm_state_out
    ...
    def save_network(self, iter=''):
        save_path = self.save_weights_path + '/drone_2D' + '_testnew_' + iter
